...
def score_universe(broker: AlpacaBroker) -> pd.Series:
    ...
    start = end - timedelta(days=MOMENTUM_LOOKBACK + 60)  # extra buffer

    scores = {}
    start_str = start.strftime("%Y-%m-%d")
    end_str = end.strftime("%Y-%m-%d")

    # Download symbols in batches to avoid timeouts and parse issues
    batch_size = 20
    all_closes = {}
    for i in range(0, len(UNIVERSE), batch_size):
        ...
        try:
            ...
            if isinstance(raw.columns, pd.MultiIndex):
                # Default yfinance format: (field, ticker) — level 0 = field
